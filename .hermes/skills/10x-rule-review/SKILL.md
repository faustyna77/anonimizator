...
Ścieżka do pliku jest przekazana po `/10x-rule-review`. Powinna wskazywać pojedynczy plik markdown (bezwzględnie, względnie względem repozytorium lub z prefiksem `@`). Przykłady:
...
Jeśli po komendzie nie podano ścieżki, użyj `clarify`, aby jednokrotnie zapytać użytkownika o ścieżkę. Nie zgaduj.

Jeśli ścieżka prowadzi do katalogu, użyj `clarify`, aby zapytać, który plik poddać przeglądowi. Jeśli prowadzi do wielu plików (np. `**/AGENTS.md`), użyj `search_files`, oceniaj je pojedynczo i raportuj każdą kartę wyników osobno — nie łącz ich.
...
1. Przeczytaj cały plik za pomocą `read_file`; jeśli ma > 2000 linii, czytaj fragmentami aż do końca.
2. Oblicz Kontrole 1–4.
3. Uruchom Kontrolę 5 w osobnym wieloetapowym przepływie (5a lista → 5b komentarz → 5c propozycja → 5d pytanie przez `clarify` → 5e przypomnienie o zmianie atomowej). Edycja zmieniająca kolejność, jeśli wystąpi, ma miejsce tutaj i tylko po wyraźnej zgodzie użytkownika.
...
Jeśli 5c utworzyło propozycję, **zapytaj użytkownika przez `clarify`** przed dotknięciem pliku. Sformułuj pytanie konkretnie. Przykładowe opcje:
